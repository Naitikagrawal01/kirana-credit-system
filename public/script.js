...
async function load() {
  ...
  document.getElementById("list").innerHTML = filtered.map(c=>`
    <div class="card">
      <b>${c.name}</b><br>₹${c.total_outstanding}
    </div>
  `).join("");

  document.getElementById("cid").innerHTML = data.map(c=>`<option value="${c._id}">${c.name}</option>`).join("");
  document.getElementById("rid").innerHTML = cid.innerHTML;
...
async function loadHistory() {
  ...
  repayList.innerHTML = repays.map(r=>`
...
async function repay() {
  await fetch("/repay", {
    ...
    headers:{"Content-Type":"application/json"},
    body: JSON.stringify({
      customer_id:rid.value,
      amount:Number(ramt.value),
      date:new Date()
    })
  });
  load();
