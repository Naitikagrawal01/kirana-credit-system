...
async function addCustomer(){await fetch("/customer",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:name.value,phone:phone.value,address:addr.value})});load()}
...
async function addCredit(){await fetch("/credit",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({customer_id:cid.value,amount:Number(amt.value),items:item.value,date:new Date()})});load()}

async function repay(){await fetch("/repay",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({customer_id:rid.value,amount:Number(ramt.value),date:new Date()})});load()}
